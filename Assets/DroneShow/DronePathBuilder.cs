...
private static List<Vector3> PadEndpoints(List<Vector3> path)
{
    ...
    padded.Add(first + (first - second));         // Extrapolated before first
    padded.AddRange(path);
    padded.Add(last + (last - secondLast));       // Extrapolated after last
...
public static DronePath FromStartToGoal(Vector3 start, Vector3 goal)
{
    ...
    // More frequent waypoints and increased curvature
    controlPoints.AddRange(WaypointGenerator.GenerateWaypointsBetween(start, goal, 20, 0.15f, 0.6f)); // More points and sharper curves

    controlPoints.Add(goal);

    controlPoints = PadEndpoints(controlPoints);  // Extrapolate for smoother start/end
    var smoothed = GenerateCatmullRomPath(controlPoints, 40);  // Higher resolution for smoother curves
